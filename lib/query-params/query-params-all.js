...
module.exports = function (typeFormat, queryParams) {
  ...
  const dateFrom = queryParams.query['filter[date[from]]'] || queryParams.query['date[from]'] || null;
  queryParamsAll.dateFrom = formatValue(typeFormat, dateFrom);

  const dateTo = queryParams.query['filter[date[to]]'] || queryParams.query['date[to]'] || null;
