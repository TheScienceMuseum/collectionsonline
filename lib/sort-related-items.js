...
module.exports = (related, id) => {
  ...
  related.forEach(el => {
    ...
    let slugValue = el._source.summary.title && slug(el._source.summary.title).toLowerCase();
    ...
    if (sortedRelated[group]) {
      ...
      const item = {
        ...
        link: config.rootUrl + '/' + TypeMapping.toExternal(el._source['@datatype'].base) + '/' + TypeMapping.toExternal(el._id) + slugValue
      };
