...
function createPeopleFacets (results) {
 // TODO extract the filter object from the aggregations of the results
  var occupationValues = [];
  results.aggregations.people.occupation.buckets.forEach(value => {
    occupationValues.push({ value: value.key, count: value.doc_count });
  });

  var placeBorn = [];
  results.aggregations.people.place_born.buckets.forEach(value => {
    placeBorn.push({ value: value.key, count: value.doc_count });
  });

  var organisation = [];
  results.aggregations.people.organisation.buckets.forEach(value => {
