...
function createCountCategories (type, results) {
  ...
  const numbers = {};
  numbers.all = results.aggregations.total.doc_count;
  buckets.forEach(bucket => {
    numbers[TypeMapping.toExternal(bucket.key)] = bucket.doc_count;
  });
  // redefine the number of the current type which depend on the filters selected
  numbers[type] = results.hits.total;
