...
module.exports = (queryParams, results, config) => {
  ...
  return { data, included, links, meta, inProduction };
};
...
const createResource = {
  object (hit, rootUrl, mediaPath) {
    ...
    const links = { self: `${rootUrl}/objects/${id}/${slug(attributes.summary_title).toLowerCase()}` };
  ...
  agent (hit, rootUrl) {
    ...
    const relationships = rels.relationships;
    const included = rels.included;
    const links = { self: `${rootUrl}/people/${id}/${slug(attributes.summary_title).toLowerCase()}` };
  ...
  archive (hit, rootUrl, mediaPath) {
    ...
    const relationships = rels.relationships;
    const included = rels.included;
    const links = { self: `${rootUrl}/documents/${id}/${slug(attributes.summary_title).toLowerCase()}` };
...
function createLinks (queryParams, results, rootUrl) {
  const totalPages = Math.ceil(results.body.hits.total / queryParams.pageSize);
...
function createMeta (queryParams, results) {
  const total = (results.body.hits || {}).total || 0;
