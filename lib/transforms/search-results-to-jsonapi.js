...
const createResource = {
  object (hit, rootUrl, mediaPath) {
    ...
    const links = { self: `${rootUrl}/objects/${id}/${slug(attributes.summary.title).toLowerCase()}` };
  ...
  agent (hit, rootUrl) {
    ...
    const relationships = rels.relationships;
    const included = rels.included;
    const links = { self: `${rootUrl}/people/${id}/${slug(attributes.summary.title).toLowerCase()}` };
  ...
  archive (hit, rootUrl, mediaPath) {
    ...
    const relationships = rels.relationships;
    const included = rels.included;
    const links = { self: `${rootUrl}/documents/${id}/${slug(attributes.summary.title).toLowerCase()}` };
...
function createLinks (queryParams, results, rootUrl) {
  const totalPages = Math.ceil(results.body.hits.total.value / queryParams.pageSize);
