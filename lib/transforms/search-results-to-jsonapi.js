...
function createMeta (params, results) {
  ...
  return { total_pages: totalPages, count: {type: countCategories}, filters: {} };
}

function createCountCategories (results) {
  const result = { all: 0, people: 0, objects: 0, documents: 0, term: 0, place: 0, event: 0 };
  const buckets = results.aggregations.total_per_categories.buckets;
  result.all = results.aggregations.total.doc_count;
  buckets.forEach(bucket => {
    result[TypeMapping.toExternal(bucket.key)] += bucket.doc_count;
  });
  return result;
}
