...
module.exports = {
  getMadeDate (data) {
    const dateValue = getNestedProperty(data, 'attributes.creation.date.0.value');
    const earliestDate = getNestedProperty(data, 'attributes.creation.date.0.from');
  ...
  getDisplayLocation (data) {
    ...
    // new model
    if (data.attributes.facility && data.attributes.facility[0] && data.attributes.facility[0]['@hierarchy']) {
      data.attributes.facility[0]['@hierarchy'].forEach((f) => {
        if (f['@datatype'] === 'permanent gallery' || f['@datatype'] === 'temporary exhibition') {
  ...
  getBirthDate (item) {
    const latestYearExtracted = getNestedProperty(item, 'attributes.birth.date.to') || getNestedProperty(item, 'attributes.birth.date.0.to') || null;
    const birthDateFreetext = getNestedProperty(item, 'attributes.birth.date.value') || getNestedProperty(item, 'attributes.birth.date.0.value') || null;
  ...
  getDeathDate (item) {
    const latestYearExtracted = getNestedProperty(item, 'attributes.death.date.to') || getNestedProperty(item, 'attributes.death.date.0.to') || null;
    const deathDateFreetext = getNestedProperty(item, 'attributes.death.date.value') || getNestedProperty(item, 'attributes.death.date.0.value') || null;
  ...
  getBorn (data, links) {
    ...
    const value = getNestedProperty(data, 'attributes.birth.location.name.0.value');
  ...
  getOccupation (data, links) {
    ...
    if (rawValue && Array.isArray(rawValue)) {
      ...
      values = [{
        ...
        link: links ? links.root + '/search/people/occupation/' + this.formatLink(encodeURIComponent(rawValue)) : null
      }];
    }
