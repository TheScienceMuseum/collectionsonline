...
module.exports = {
    ...
    if (locations) {
      locations.forEach(loc => {
        if (loc.type === 'gallery' || loc.type === 'museum') {
          if (loc.value === 'National Media Museum') {
            display[loc.type] = 'National Science and Media Museum';
          } else {
  ...
  getBirthDate (item) {
    var latestYearExtracted = getNestedProperty(item, 'attributes.lifecycle.birth.0.date.0.latest');
    var birthDateFreetext = getNestedProperty(item, 'attributes.lifecycle.birth.0.date.0.value');
    if (birthDateFreetext) {
      if (birthDateFreetext.includes('BC') || birthDateFreetext.includes('BCE') || birthDateFreetext.includes('CE')) {
  ...
  getDeathDate (item) {
    var latestYearExtracted = getNestedProperty(item, 'attributes.lifecycle.death.0.date.0.latest');
    var deathDateFreetext = getNestedProperty(item, 'attributes.lifecycle.death.0.date.0.value');
    if (deathDateFreetext) {
      if (deathDateFreetext.includes('BC') || deathDateFreetext.includes('BCE') || deathDateFreetext.includes('CE')) {
