...
module.exports = function (queryParams, rootUrl) {
  ...
  const facetsImage = ['filter[has_image]', 'filter[image_license]'];
  links['filter[images]'] = `${rootUrl}/search${type}?${QueryString.stringify(ignoreQueryParam(facetsImage, queryParams.query))}`;
  return links;
};
