...
module.exports = function (queryParams, rootUrl) {
  ...
  facetsName.forEach(function (facet) {
    links[facet] = `${rootUrl}/search${type}${paramify(ignoreQueryParam([facet, removeFilter(facet)], queryParams.query))}${querify(ignoreQueryParam([facet, removeFilter(facet)], queryParams.query))}`;
  });
  ...
  links.dates = `${rootUrl}/search${type}${paramify(ignoreQueryParam(facetsDate, queryParams.query))}${querify(ignoreQueryParam(facetsDate, queryParams.query))}`;

  // special link for images: ignore has_image AND image_license
  const facetsImage = ['has_image', 'image_license'];
  links['filter[images]'] = `${rootUrl}/search${type}${paramify(ignoreQueryParam(facetsImage, queryParams.query))}${querify(ignoreQueryParam(facetsImage, queryParams.query))}`;

  // special link for on display: ignore museum AND gallery
  const musemFacets = ['filter[museum]', 'filter[gallery]', 'museum', 'gallery'];
  links['filter[museum]'] = `${rootUrl}/search${type}${paramify(ignoreQueryParam(musemFacets, queryParams.query))}${querify(ignoreQueryParam(musemFacets, queryParams.query))}`;
