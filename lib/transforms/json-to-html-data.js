...
module.exports = (resource) => {
  ...
  const titlePage = normalise(getValues.getTitle(resource.data), { system: { value: system }, type }) + ' | Science Museum Group Collection';
...
function getDisplayLocation (data) {
  ...
  if (data.attributes.facility && data.attributes.facility[0]['@hierarchy']) {
    // console.log(data.attributes.facility[0]['@hierarchy']);
    let site = '';
    let gallery = '';
...
function getWikiLink (data) {
  ...
  const note = (data.attributes.description || []).find(el => el.value.indexOf('wiki') > -1);
...
function getMaterials (data) {
  ...
  if (data.attributes.material) {
    values = data.attributes.material.map(material => {
      return {
        value: material.value,
        link: `/search/objects/material/${getValues.formatLink(material.value)}`
      };
    });
    ...
    images = data.attributes.multimedia.map(e => {
      if (e.processed) {
        ...
        if (data.type === 'documents') {
          ...
          if (description) {
            const desc = description.primary || description.web || { initialDescription: [''] };
