...
module.exports = (resource) => {
  ...
  return {title, type, fact, related, description, details, displayLocation, system};
};
...
function getFacts (resource) {
  ...
  return [occupation, nationality, born, birth, death, made].concat(makers).filter(el => el);
}
...
function getDetails (data) {
  ...
  const access = getAccess(data);
  const legal = getLegal(data);
  const arrangement = getArrangement(data);
  const notes = getNotes(data);

  return [website, category, accession, materials, measurements, status, identifier, access, legal, arrangement].concat(notes).filter(el => el);
}
...
function getMeasurements (data) {
  const value =
    getNestedProperty(data, 'attributes.component.0.measurements.display') ||
    getNestedProperty(data, 'attributes.measurements.dimensions.0.value');
...
function getLegal (data) {
  const value = getNestedProperty(data, 'attributes.legal.rights.0.details');
  return value ? {key: 'Legal', value: value} : null;
}
