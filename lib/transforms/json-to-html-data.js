...
function getDisplayLocation (data) {
  const location = getValues.getDisplayLocation(data);
  const displayValue = '/search/museum' + location.site + '/gallery' + location.gallery;
  const linkValue = location.site + ' : ' + location.gallery;
  return displayValue ? { key: 'Display Location', displayValue, link: linkValue } : null;
}
...
function doChildRecordsHaveImages (childRecords) {
  let childRecordsHaveImages = false;
  childRecords.forEach((c) => {
    // console.log('has images' + c.hasImage);
    if (c.hasImage) {
      childRecordsHaveImages = true;
    }
  });
...
function getChildRecords (
  ...
  if (childRecords.length === 0 || depth > 3) {
    return [];
  }
