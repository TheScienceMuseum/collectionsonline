...
function getChildRecords (
  ...
  return childRecords.map((record) => {
    ...
    const recordLinks = getLinks(record.data.attributes);
    const detailsArr = getChildRecordDetails(record, recordLinks);
    ...
    let children = [];
    let childRecordsNoImages = false;

    if (!images.length) {
      childRecordsNoImages = true;
    }
