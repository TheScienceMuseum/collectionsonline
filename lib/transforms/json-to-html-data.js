...
module.exports = (resource) => {
  ...
  return {title, titlePage, type, fact, related, description, details, displayLocation, system, level, date, links, identifier, parent, fonds, images};
};
...
function getObjectType (data, links) {
  ...
  if (data.type !== 'people' && data.attributes.name) {
    ...
    return value ? {key: 'type', value: value, link: links.root + '/search/objects?filter[type]=' + value} : null;
  }
}
...
function getImages (data) {
  if (data.attributes.multimedia) {
    return data.attributes.multimedia.map(e => {
