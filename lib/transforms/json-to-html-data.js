...
module.exports = (resource) => {
  const masthead = getMasthead(resource.data);
  ...
  const breadcrumbLD = getJSONLD.breadcrumb(resource);
  ...
  const titlePage =
    getValues.getTitle(resource.data) + ' | Science Museum Group Collection';
  const metaDescription = buildRecordMetaDescription(resource.data, title, type, date, description);
  const ogType = type === 'people' ? 'profile' : 'article';
  ...
  const wikidata = getWikiLink(resource.data);
  ...
  const dublinCore = buildDublinCore(resource, type, title, date, description);
...
function buildRecordMetaDescription (data, title, type, date, description) {
  const MAX_LENGTH = 500;

  // Prefer the full first paragraph of description text
  const descText = extractDescriptionText(description);
  if (descText) {
    return descText.length > MAX_LENGTH ? descText.substring(0, MAX_LENGTH - 3) + '...' : descText;
  }

  // Fall back to a generated sentence when no description text exists
  switch (type) {
    case 'people': {
      const occupation = getNestedProperty(data, 'attributes.occupation');
      const occText = occupation && occupation[0] ? occupation[0].value : null;
      return occText
        ? `Discover ${title}, ${occText}, in the Science Museum Group Collection.`
        : `Discover ${title} in the Science Museum Group Collection.`;
    }
    case 'objects': {
      const dateStr = date ? ` (${date})` : '';
      return `Explore ${title}${dateStr} in the Science Museum Group Collection.`;
    }
    case 'documents':
      return `Explore ${title} in the Science Museum Group Collection archives.`;
    case 'group':
      return `Explore ${title} in the Science Museum Group Collection.`;
    default:
      return `Explore ${title} in the Science Museum Group Collection.`;
  }
}
...
function extractDescriptionText (description) {
  ...
  const source = description.primary || description.web || description.briefbiog;
  if (!source || !source.initialDescription) return null;
  // initialDescription is an array of strings (paragraph lines)
...
function getMasthead (data) {
  const masthead = [];
  // add hardcoded entry for Building One
  // should come from the index or a fixture longer term
  if (data.attributes['@admin']?.uid === 'c83918') {
    masthead.image = 'https://collection.sciencemuseumgroup.org.uk/assets/img/explore/banner/1600/hawkins-building-wide.jpg';
    masthead.title = 'Hawking Building Freestanding Grid';
    masthead.shade = 'dark';
  }
  return masthead.image ? masthead : null;
}

function getSketchFab (data) {
  const values = [];
  if (data.attributes.enhancement && data.attributes.enhancement.web) {
    if (!Array.isArray(data.attributes.enhancement.web)) return;
    data.attributes.enhancement.web.forEach((el) => {
      if (el && el.platform && el.platform === 'sketchfab') {
        values.push(el);
      }
    });
...
function getYouTube (data) {
  const values = [];
  if (data.attributes?.enhancement?.web && Array.isArray(data.attributes.enhancement.web)) {
    data.attributes.enhancement.web.forEach((el) => {
      if (el?.platform && el.platform === 'youtube') {
        values.push(el);
      }
    });
    return values.length ? values : null;
  ...
  if (data.attributes?.enhancement?.web && Array.isArray(data.attributes.enhancement.web)) {
    data.attributes.enhancement.web.forEach((el) => {
      if (el?.platform && el.platform === '3D') {
        const modifiedEl = { ...el, platform: 'rotational' };
        values.push(modifiedEl);
      }
    });
    return values.length ? values : null;
  }
...
function getAudio (data) {
  ...
  if (data.attributes?.enhancement?.web && Array.isArray(data.attributes.enhancement.web)) {
    data.attributes.enhancement.web.forEach((el) => {
      ...
      const processedEl = { ...el };

      if (el.platform === 'oralhistory') {
        processedEl.prefix = 'https://smgco-oralhistories.s3-eu-west-1.amazonaws.com';
        values.push(processedEl);
      } else if (el.platform === 'audio') {
        processedEl.prefix = 'https://smgco-audio.s3-eu-west-1.amazonaws.com';
        processedEl.audiofile = el.audiofile || `${processedEl.prefix}/${el.id}`;
        values.push(processedEl);
      }

      if (el.thumbnail && !urlRegex.test(el.thumbnail)) {
        processedEl.thumbnail = `${processedEl.prefix}/${el.thumbnail}`;
      }
    });
    return values.length ? values : null;
  }
...
function getWikiLink (data) {
  return normaliseWikidata.getQCode(data.wikidata);
}
...
function getAIRelated (resource, type) {
  return resource.data.relatedAIItems;
}
...
function getRelated (resource, type) {
  if (resource.data.relationships) {
    ...
    if (related.people) {
      // Pre-build a lowercase set of all allowed role values for fast lookup.
      const allowedRoles = new Set(
        [...usersList, ...associationList].map((r) => r.toLowerCase())
      );

      // Normalise each entry: lowercase the role if allowed, null it otherwise.
      // Spread into new objects to avoid mutating shared included references.
      // Deduplication is guaranteed upstream (getRelationships) so no seen-map needed here.
      related.people = related.people
        .filter(Boolean)
        .map((e) => {
          const roleValue = getNestedProperty(e, 'attributes.role.value');
          const roleLower = roleValue ? String(roleValue).toLowerCase() : null;
          const role = (roleLower && allowedRoles.has(roleLower))
            ? { ...e.attributes.role, value: roleLower }
            : null;
          const displayTitle = getValues.getTitle(e) ||
            getNestedProperty(e, 'attributes.summary.title');
          return { ...e, attributes: { ...e.attributes, role }, displayTitle };
        })
        .sort((a, b) => {
          const nameA = (getNestedProperty(a, 'attributes.summary.title') || '').toLowerCase();
          const nameB = (getNestedProperty(b, 'attributes.summary.title') || '').toLowerCase();
          return nameA < nameB ? -1 : nameA > nameB ? 1 : 0;
        });
    }
    if (related.organisations) {
...
function getDetails (resource, links) {
  ...
  const extent = getExtent(resource.data);
...
function getMade (resource, links) {
  ...
  const placeLinks =
    ...
    places.map(function (p) {
      const unknown = p.match(/unknown|unidentified|unattributed/i);
      return {
        value: unknown ? unknown[0] : p,
        link: unknown ? null : links.root + '/search/places/' + encodeFilterValue(p)
      };
    });
...
function getCategory (data, links) {
  ...
  if (data.attributes.category) {
    ...
    return {
      ...
      link: links.root + '/search/categories/' + encodeFilterValue(values.toString())
    };
  } else {
    return null;
  }
...
function getCollection (data, links) {
  ...
  if (values.length) {
    return {
      key: 'Collection',
      value: values.toString(),
      link: links.root + '/search/collection/' + encodeFilterValue(values.toString())
    };
  } else {
    return null;
  }
  ...
  if (materialData) {
    values = materialData.map((material) => {
      ...
      const links = Array.isArray(value)
        ? value.map((v) => `/search/objects/material/${encodeFilterValue(v)}`)
        : [`/search/objects/material/${encodeFilterValue(value)}`];
      return {
        value,
        link: links
      };
...
function getExtent (data) {
  if (data.type !== 'documents' || !data.attributes.measurements || !data.attributes.measurements.extent) {
    return null;
  }
  return { key: 'Extent', value: data.attributes.measurements.extent };
}

function getMeasurements (data) {
  const values = [];

  // archive documents — format dimensions grouped by part
  if (data.attributes.measurements && data.type === 'documents') {
    if (data.attributes.measurements.dimensions) {
      const partGroups = {};
      data.attributes.measurements.dimensions.forEach(function (el) {
        const part = el.part || 'Whole';
        if (!partGroups[part]) {
          partGroups[part] = [];
        }
        partGroups[part].push(el);
      });

      const parts = Object.keys(partGroups);
      const singleWhole = parts.length === 1 && parts[0] === 'Whole';

      parts.forEach(function (part) {
        if (!singleWhole) {
          values.push('<div>' + part + '</div>');
        }
        partGroups[part].forEach(function (dim) {
          const indent = singleWhole ? '' : ' style="margin-left:1em"';
          if (dim.value && dim.units && dim.dimension) {
            values.push('<div' + indent + '>' + dim.value + dim.units + ' (' + dim.dimension + ')</div>');
          } else if (dim.value) {
            values.push('<div' + indent + '>' + dim.value + '</div>');
          }
        });
      });
    }
...
function getLegal (data) {
  ...
  if (getNestedProperty(data, 'attributes.legal')) {
    Object.keys(data.attributes.legal).forEach((el) => {
      ...
      legals.push(value ? { key, value } : null);
    });
  }
  ...
  if (data.type !== 'people' && objectTypeData) {
    value = objectTypeData
      ...
      .map((el) => {
        ...
        return {
          value: val,
          link: links.root + '/search/objects/object_type/' + encodeFilterValue(val)
        };
      });
      ...
      if (el.attributes.hierarchy) {
        el.attributes.hierarchy.forEach((el) => {
          taxonomy.push({
            ...
            link: links.root + '/search/objects/object_type/' + encodeFilterValue(el.name[0].value)
          });
        });
      }
    });
...
function getParent (resource) {
  if (getNestedProperty(resource, 'data.relationships.parent')) {
    ...
    // if this is an object recordm then only show link if it's parent is a MPH record
    ...
    images = data.attributes.multimedia.map((e) => {
      if (e['@processed']) {
        const img = {
          ...
          card: getNestedProperty(e, '@processed.large_thumbnail.location'),
...
// if resolves to true on a part record, allows for special styling
...
function buildDublinCore (resource, type, title, date, description) {
  const data = resource.data;
  const dc = { title };

  // DC.type — DCMI Type Vocabulary
  switch (type) {
    case 'objects': dc.type = 'PhysicalObject'; break;
    case 'people': dc.type = 'Agent'; break;
    case 'documents': dc.type = 'Text'; break;
    case 'group': dc.type = 'Collection'; break;
  }
  ...
  // DC.identifier — accession number
  const identifiers = data.attributes && data.attributes.identifier;
  if (identifiers) {
    identifiers.forEach(function (el) {
      if (el.type === 'accession number') {
        dc.identifier = el.value;
      }
    });
  }

  // DC.description — plain text, truncated
  const descText = extractDescriptionText(description);
  if (descText) {
    dc.description = descText.length > 200 ? descText.substring(0, 197) + '...' : descText;
  }

  // DC.creator — makers (objects/documents only)
  if (type === 'objects' || type === 'documents') {
    const makers = getValues.getMakers(resource);
    if (makers && makers.length) {
      const names = makers
        .filter(function (m) { return m.value && !m.value.match(/^unknown|^unidentified|^unattributed/i); })
        .map(function (m) { return m.value; });
      if (names.length) {
        dc.creator = names.join(', ');
      }
    }
  }

  // DC.subject — category name
  const category = getNestedProperty(data, 'attributes.category.0.name');
  if (category) {
    dc.subject = category;
  }

  // DC.format — materials (objects only)
  if (type === 'objects') {
    const materialData = data.attributes && data.attributes.material;
    if (materialData && materialData.length) {
      dc.format = materialData.map(function (m) { return m.value; }).join(', ');
    }
  }

  // DC.rights — catalogue data rights (not image rights)
  dc.rights = 'The Board of Trustees of the Science Museum';

  // DC.relation — Wikidata sameAs (people only)
  if (type === 'people') {
    const wikidataUrl = normaliseWikidata.getUrl(data.wikidata);
    if (wikidataUrl) {
      dc.relation = wikidataUrl;
    }

    // DC.description override for people — use occupation
    const occupation = getNestedProperty(data, 'attributes.occupation');
    if (occupation && occupation.length) {
      const occText = occupation.map(function (o) { return o.value; }).join(', ');
      dc.description = occText;
    }
  }

  // DC.publisher — always Science Museum Group
  dc.publisher = 'Science Museum Group';
