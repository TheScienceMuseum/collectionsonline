...
module.exports = (resource) => {
  ...
  return {title, titlePage, type, fact, related, description, details, displayLocation, system, level, date, links, identifier, parent, fonds};
};
...
function getOccupation (data, links) {
  ...
  return value ? {key: key, value: value, link: links.root + '/search?occupation=' + rawValue} : null;
}
...
function getBorn (data, links) {
  ...
  return value ? {key: key, value: value, link: links.root + '/search?birth[place]=' + value} : null;
}
...
function getCategory (data, links) {
  ...
  if (data.attributes.categories) {
    ...
    return {key: 'Category', value: values.toString(), link: links.root + '/search?categories=' + values.toString()};
  } else {
    return null;
  }
}
