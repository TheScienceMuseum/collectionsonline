...
module.exports = (resource) => {
  ...
  // console.log(related.objects.map(obj => obj.attributes), "checling the attributes")
...
function getUID (data) {
  const adminData = data.attributes ? data.attributes.admin : data.admin;
...
function getMeasurements (data) {
  ...
  if (data.attributes.measurements && data.type === 'objects') {
    if (data.attributes.measurements.display) {
      var measurements = data.attributes.measurements.display;
      measurements = measurements.replace(/,$/, ''); // strip stray comma, see issue #1529
      values.push(measurements + '<br>');
...
function getObjectType (data, links) {
  ...
  if (data.type !== 'people' && objectTypeData) {
    ...
    return value ? { key: 'type', value } : null;
  }
}
        ...
        if (data.type === 'documents') {
          ...
          img.title =
            e.source && e.source.title
              ? e.source.title.find((el) => el.type === 'main title').value ||
                e.source.title[0].value
              : '';
...
function formatLicense (license) {
  ...
  if (license.details === 'unknown') {
    license.details = 'Copyright holder unknown';
  }
  return { details: license.details, usage_terms: formatted, cc, ccurl, image };
}
...
function getChildRecords (data, links) {
  ...
  if (childRecords.length) {
    return childRecords.map((record) => {
      ...
      const link = record.data.attributes.link;
      return getGroupingRecordData(record, groupingType, description, title, details, images, inProduction, link, links);
    });
  }
  return null;
}
...
function getGroupingRecordData (record, groupingType, description, title, details, images, inProduction, link, links) {
  if (groupingType === 'SPH') {
    // SPH specific attributes
    return {
      images,
      title,
      details,
  ...
  } else if (groupingType === 'MPH') {
    // MPH specific attributes
