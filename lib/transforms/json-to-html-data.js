...
function getRotational (data) {
  ...
  if (data.attributes.enhancement && data.attributes.enhancement.web) {
    ...
    // dummy data:
    var fakeData = {
      id: 'smgco-360/28f0ba52-6deb-43c0-a2e5-1fb82c5247cd',
      platform: 'rotational',
      title: 'Apple I Personal Computer'
    };
    values.push(fakeData);
    return values.length ? values : null;
  }
}
...
function getDetails (resource, links) {
  ...
  return [website, category, accession, materials, measurements, identifier, subject, access, arrangement, objectType, taxonomy].concat(legal, notes).filter(el => el);
}
