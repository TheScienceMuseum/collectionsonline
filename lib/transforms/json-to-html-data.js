...
function getMade (resource, links) {
  ...
  const placeLinks = places && places.map(function (p) {
    return {
      value: p,
      link: links.root + '/search?filter[places]=' + encodeURIComponent(p)
    };
  });
...
function getAccession (data) {
  ...
  if (data.attributes.identifier) {
    ...
    return value ? {key: 'Object Number', value: value, link: '/oid/' + encodeURIComponent(value) + '?redirect=true'} : null;
  } else {
    return null;
  }
}
...
function getObjectType (data, links) {
  ...
  if (data.type !== 'people' && data.attributes.name) {
    value = data.attributes.name.map((el, i, arr) => {
      ...
      return {
        value: val,
        link: links.root + '/search/objects?filter[type]=' + encodeURIComponent(val)
      };
    });
