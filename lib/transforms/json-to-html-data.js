...
module.exports = (resource) => {
  ...
  return {title, type, fact, related, description, details, displayLocation, system, level};
};
...
function getMeasurements (data) {
  ...
  if (data.attributes.component) {
    data.attributes.component.forEach((el, i, arr) => {
      value.push(el.measurements.display + '<br>');
    });
    return value.length ? {key: 'Measurements', value: value.join('')} : null;
  }
}
