...
function getParent(resource) {
  if (getNestedProperty(resource, 'data.relationships.parent')) {
    ...
    // hard coded check for Daily Herald record - see ticket https://github.com/TheScienceMuseum/collectionsonline/issues/1651
    // the parent link of DHA has a redirect to its search collection, so the link from a child currently throws a server error
...
function getGroupDisplayData(resource) {
  ...
  const title = grouping?.[0]?.summary?.title;
  const uid = grouping?.[0]?.['@admin']?.uid;
  if (grouping?.[0]?.['@link']?.type === 'MG') {
    return {
      title,
      uid,
    };
