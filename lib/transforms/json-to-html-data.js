...
function buildRecordMetaDescription (data, title, type, date, description) {
  ...
  // Extract plain text from the description object
  ...
  switch (type) {
    ...
    case 'objects': {
      ...
      desc = `Explore ${title}${dateStr} in the Science Museum Group Collection.`;
      break;
    }
    case 'documents': {
      desc = `Explore ${title} in the Science Museum Group Collection archives.`;
      break;
    }
    case 'group': {
      desc = `Explore ${title} in the Science Museum Group Collection.`;
      break;
    }
    default:
      desc = `Explore ${title} in the Science Museum Group Collection.`;
  ...
  // Append a snippet of the description text if we have room
  if (descText && desc.length + 1 + descText.length <= MAX_LENGTH) {
    desc += ' ' + descText;
  } else if (descText && desc.length < MAX_LENGTH - 20) {
    const remaining = MAX_LENGTH - desc.length - 4; // space + ...
    desc += ' ' + descText.substring(0, remaining) + '...';
  }

  return desc.length > MAX_LENGTH ? desc.substring(0, MAX_LENGTH - 3) + '...' : desc;
}
