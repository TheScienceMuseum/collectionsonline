...
function getDetails (resource, links) {
  ...
  const transcription = getTranscription(resource.data);
  ...
  return [website, category, collection, accession, materials, measurements, identifier, subject, access, arrangement, transcription, objectType, taxonomy].concat(legal, notes).filter(el => el);
}
...
function getMeasurements (data) {
  ...
  } else if (data.attributes.measurements) {
    return { key: 'Measurements', value: data.attributes.measurements.display + '<br>' };
...
function getTranscription (data) {
  const value = getNestedProperty(data, 'attributes.transcription.system.0');
  return value ? { key: 'Transcription', value: value } : null;
}
