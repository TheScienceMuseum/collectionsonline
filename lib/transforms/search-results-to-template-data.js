...
module.exports = (queryParams, results, config) => {
  ...
  const data = {
    ...
    q: queryParams.q,
    results: results.data.map(createResult, config),
...
function createResult (item) {
  return {
    ...
    ...(item.type === 'group' ? { collage: getCollageData(item, this.mediaPath) } : {}),
...
function getCollageData (item, mediaPath) {
  ...
  const firstImageArray = filteredImages
    .map((child) =>
      getNestedProperty(child, 'multimedia.@processed.medium_thumbnail.location')
    )
  ...
  // creates full URL, like the figures
  images = images.map((image) => mediaPath + image);
  return images;
}
