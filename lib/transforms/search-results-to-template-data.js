...
module.exports = (queryParams, results, config) => {
  ...
  const museumFilters = Object.keys(selectedFilters.museum || {});
  const archiveFilters = Object.keys(selectedFilters.archive || {});
  const clearCategoryLink = createClearFilterCategoryLink(queryParams, rootUrl, 'categories');
  const clearMuseumLink = createClearFilterCategoryLink(queryParams, rootUrl, 'museum');
  const clearArchiveLink = createClearFilterCategoryLink(queryParams, rootUrl, 'archive');
  const data = {
...
function createClearFilterCategoryLink (queryParams, rootUrl, filter) {
  const params = {};
