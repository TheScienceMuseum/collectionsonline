...
module.exports = (queryParams, results, config) => {
  ...
  const clearCollectionLink = createClearStickyFilterLink(queryParams, rootUrl, /collection/);
  ...
  const clearImageLink = {
    ...
    imageLicense: createClearStickyFilterLink(queryParams, rootUrl, /image_license/)
  };
  ...
  const data = {
    ...
    inProduction,
    descriptionBox: createDescriptionBox(selectedFilters),
...
function getCategoryOrArtist (item) {
  if (getCategory(item) === 'Art' || getCategory(item) === 'Contemporary Art Collection') {
    if (getNestedProperty(item, 'attributes.lifecycle.creation.0.maker.0.summary_title')) {
      return getNestedProperty(item, 'attributes.lifecycle.creation.0.maker.0.summary_title');
    }
  }
...
function getFigure (item, config) {
  const location = getNestedProperty(item, 'attributes.multimedia.0.processed.large_thumbnail.location');
...
function createPagination (queryParams, results, rootUrl) {
  ...
  const limitNbPage = results.meta.total_pages <= 10 ? results.meta.total_pages : 10;
  ...
  const pages = Pagination.pages(
    ...
    limitNbPage,
    {
      createPageLink: (page) => {
        ...
        return `${rootUrl}/search${type}${paramify(pageParams)}${querify(pageParams)}`;
      }
    }
  );
