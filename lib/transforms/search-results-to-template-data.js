...
function createClearFilterLink (queryParams, rootUrl) {
  ...
  return `${rootUrl}/search${type}?q=${QueryString.stringify(params)}`;
}
...
function createSelectedFilters (queryParams) {
  var selectedFilters = {};
  const type = queryParams.type === 'all' ? 'objects' : queryParams.type;
  const facets = queryParams.filter[type];
  Object.keys(facets).forEach(filter => {
    if (typeof facets[filter] === 'string' && filter !== 'q') {
      var filters;
      if (facets[filter].indexOf(',') > -1) {
        filters = facets[filter].split(',');
      } else {
