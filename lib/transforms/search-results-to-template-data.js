...
module.exports = (queryParams, results, config) => {
  ...
  console.log('################Q U E R Y ###############');
  config = config || {};
  const rootUrl = config.rootUrl || '';
  const selectedFilters = createSelectedFilters(queryParams);
  const categoryFilters = Object.keys(selectedFilters.categories || {});
  const clearCategoryLink = createClearFilterCategoryLink(queryParams, rootUrl);
  console.log(clearCategoryLink);
  console.log('^^^^^^^^^^^^^^^^^^^^^^^^^^');
  const data = {
...
function createClearFilterCategoryLink (queryParams, rootUrl) {
  const params = {};
  Object.keys(queryParams.query).forEach(function (qp) {
    if (qp !== 'filter[categories]') {
      console.log('AAADDDDD', qp);
