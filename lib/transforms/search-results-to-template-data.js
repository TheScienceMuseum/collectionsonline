...
function createPagination (params, results, rootUrl) {
  ...
  let pages = getPaginationPages(pageNumber, results.meta.total_pages);

  // Add links to pages
  pages = pages.map((page) => {
    if (page) {
      const pageParams = Object.assign({}, params, { 'page[number]': page.pageNumber });
      page.link = `${rootUrl}/search?${QueryString.stringify(pageParams)}`;
...
function getPaginationPages (currentPageNumber, totalPages, maxLinks = 6) {
  if (totalPages < maxLinks) {
    return range(0, totalPages)
      .map((n) => ({ pageNumber: n, isCurrent: currentPageNumber === n }));
  }

  if (totalPages - currentPageNumber - 1 < maxLinks) {
    const start = totalPages - maxLinks;

    return range(start, start + maxLinks)
      .map((n) => ({ pageNumber: n, isCurrent: currentPageNumber === n }));
  }
  ...
  return range(currentPageNumber, currentPageNumber + (maxLinks - 2))
    ...
    .concat([
      null,
      {pageNumber: totalPages - 2, isCurrent: false},
      {pageNumber: totalPages - 1, isCurrent: false}
    ]);
}
