...
// Number of top-scoring documents per shard to use when building facet counts
// for free-text queries. With a typical 5-shard index this samples ~1000 docs.
const SAMPLER_SHARD_SIZE = 200;

module.exports = function (queryParams, isFreeText) {
  ...
  const aggregationObjects = {
    ...
    aggs: {
      ...
      place: {
        ...
        aggs: {
          place_filters: {
            terms: { field: 'creation.place.summary.title.keyword', exclude: '.*[;,].*|[Uu]nknown.*' }
          }
        }
      },
      user: {
      ...
      place_born: {
        ...
        aggs: {
          place_born_filters: {
            terms: { field: 'birth.place.name.value.keyword', exclude: '.*;.*|[Uu]nknown.*' }
          }
        }
      },
      organisation: {
        ...
        aggs: {
          organisations_filters: {
            terms: { field: '@datatype.actual', include: 'person|organisation' }
          }
        }
      },
      count: {
  ...
  // When there is a free-text query, wrap all facet sub-aggregations in a
  // sampler so counts reflect the top-scoring results rather than the full
  // long-tail of weakly-matching documents. Browse/filter-only requests use
  // the raw aggregation so facet counts remain accurate across all matches.
  if (isFreeText) {
    aggregationObjects.aggs = {
      sample: {
        sampler: { shard_size: SAMPLER_SHARD_SIZE },
        aggs: aggregationObjects.aggs
      }
    };
  }
