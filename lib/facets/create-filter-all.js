...
module.exports = function (queryParams, filters) {
  ...
  const result = {
    bool: {
      filter: [{ terms: { '@datatype.base': categories } }]
    }
  };
  result.bool.filter = result.bool.filter.concat(
    removeFilterType(filters.people),
    removeFilterType(filters.objects),
    removeFilterType(filters.documents),
    removeFilterType(filters.group)
  );
...
function removeFilterType (filters) {
  if (filters.bool.filter) {
    return filters.bool.filter.filter((el) => {
      return !el.term;
    });
  }
}
...
function selectedFiltersCategories (queryParams) {
  ...
  if (hasFilters(queryParams.filter.group)) {
    categories.push('group');
  }
  // if non of the filters are selected select all the type by default
  // To show mgroup (Topics) records under the All tab, add 'group' back here
