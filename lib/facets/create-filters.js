...
module.exports = function (queryParams, filterType) {
  ...
  if (dateFrom && !(isNaN(dateFrom))) {
    if (dateTo) {
      filterDate.bool.should.push({ range: { 'lifecycle.death.date.latest': { 'gte': dateFrom } } }, { range: { 'lifecycle.creation.date.latest': { 'gte': dateFrom } } });
    } else {
      filterDate.bool.should.push({ range: { 'lifecycle.birth.date.latest': { 'gte': dateFrom } } }, { range: { 'lifecycle.creation.date.latest': { 'gte': dateFrom } } });
    }
  }

  if (dateTo && !(isNaN(dateTo))) {
    if (dateFrom) {
      filterDate.bool.should.push({ range: { 'lifecycle.birth.date.latest': { 'lte': dateTo } } }, { range: { 'lifecycle.creation.date.latest': { 'lte': dateTo } } });
    } else {
      filterDate.bool.should.push({ range: { 'lifecycle.death.date.latest': { 'lte': dateTo } } }, { range: { 'lifecycle.creation.date.latest': { 'lte': dateTo } } });
    }
  }
