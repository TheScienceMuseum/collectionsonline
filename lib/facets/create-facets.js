...
module.exports = function createAllFacets (results) {
  const a = results.body.aggregations.all || results.body.aggregations.people || results.body.aggregations.objects || results.body.aggregations.documents;
  return {
    category: a.category.category_filters.buckets.map(v => { return {value: v.key, count: v.doc_count}; }),
    collection: a.collection.collection_filters.buckets.map(v => { return {value: v.key, count: v.doc_count}; }),
    ...
    museum: a.museum.museum_filters.buckets.map(v => {
      if (v.key === 'Science Museum' ||
          ...
          v.key === 'Locomotion') {
        var displayValue = v.key;
        if (v.key === 'National Railway Museum') displayValue = 'Railway Museum';
        if (v.key === 'National Media Museum') displayValue = 'National Science and Media Museum';
        if (v.key === 'Museum of Science and Industry') displayValue = 'Science and Industry Museum';
        return {
          value: v.key,
