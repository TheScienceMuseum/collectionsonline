...
module.exports = function createAllFacets (results) {
  ...
  // When a sampler aggregation is present (free-text queries), facet buckets
  // are one level deeper under the `sample` key. Fall back to the raw object
  // for browse/filter-only requests where no sampler is used.
  const a = aRaw.sample || aRaw;
  return {
    ...
    organisation: a.organisation.organisations_filters.buckets
      .filter((v) => v.key === 'person' || v.key === 'organisation')
      .map((v) => {
        return { value: v.key, count: v.doc_count };
      }),
    ...
    museum: a.museum.museum_filters.buckets
      .map((v) => {
        if (
          ...
          v.key === 'Railway Museum' ||
          v.key === 'National Railway Museum' ||
          v.key === 'Science and Industry Museum' ||
