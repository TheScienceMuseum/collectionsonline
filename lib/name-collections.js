    ...
    aggs: {
      collections: {
        terms: {
          ...
          exclude: '.*;.*',
          min_doc_count: widgetConfig.minDocCount || 1
        }
      }
    }
  };
...
function getDescription (collectorName) {
  ...
  const firstSentence = match ? match[1].trim() : desc;

  // Belt-and-braces cap for long sentences: break at a word boundary if
  // possible, strip trailing punctuation, then append an ellipsis.
  if (firstSentence.length <= DESCRIPTION_MAX) return firstSentence;
  const cut = firstSentence.slice(0, DESCRIPTION_MAX);
  const boundary = cut.lastIndexOf(' ');
  const base = boundary > DESCRIPTION_MAX * 0.8 ? cut.slice(0, boundary) : cut;
