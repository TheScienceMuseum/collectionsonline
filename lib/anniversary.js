...
async function getObjectAnniversaries (elastic, config, today, overrides) {
  ...
  // Use a date-based seed so ES returns a different random ordering each day.
  // random_score is the primary driver — this prevents popular objects from
  // always dominating and ensures genuine daily variety.
  ...
  // Fetch a large pool so the seeded random pick has real variety to work with.
  const fetchSize = Math.max(remaining * 15, 50);
