...
module.exports.secondsUntilNextPeriodUTC = secondsUntilNextPeriodUTC;
...
function secondsUntilNextPeriodUTC (periodHours) {
  const ph = periodHours || widgetConfig.cache.refreshPeriodHours || 4;
  const now = new Date();
  const currentHour = now.getUTCHours();
  const nextPeriodHour = (Math.floor(currentHour / ph) + 1) * ph;
  const next = new Date(now);
  if (nextPeriodHour >= 24) {
    next.setUTCDate(next.getUTCDate() + 1);
    next.setUTCHours(0, 0, 0, 0);
  } else {
    next.setUTCHours(nextPeriodHour, 0, 0, 0);
  }
...
async function getObjectAnniversaries (elastic, config, today, overrides) {
  ...
  // Seed rotates every refreshPeriodHours (default 4h) so results change
  // multiple times a day, pulling from a genuinely varied pool of 500k+ objects.
  const periodHours = (widgetConfig.cache && widgetConfig.cache.refreshPeriodHours) || 4;
  const periodIndex = Math.floor(today.getUTCHours() / periodHours);
  const dateSeed = today.getUTCFullYear() * 100000 +
    (today.getUTCMonth() + 1) * 10000 +
    today.getUTCDate() * 100 +
    periodIndex;
  ...
  if (cfg.preferOnDisplay) {
    functions.push({
      ...
      weight: cfg.onDisplayBoost || 1.5
    });
  }

  // Large pool ensures genuine variety across 500k+ objects.
  const fetchSize = Math.max(remaining * 30, 100);
...
function getImage (source, config) {
  ...
  // multimedia is an array in the ES schema — sort by position then upload date
