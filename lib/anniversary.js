...
async function getObjectAnniversaries (elastic, config, today, overrides) {
  ...
  // Daily shuffle: seed with today's date so results vary day-to-day
  // but stay consistent within the same day (aligns with midnight cache)
  const dateSeed = today.getFullYear() * 10000 +
    (today.getMonth() + 1) * 100 + today.getDate();
  functions.push({
    random_score: { seed: dateSeed, field: '_seq_no' }
  });
...
async function getPeopleAnniversaries (elastic, config, today, overrides) {
  ...
  let allItems = overrideItems.concat(items);
