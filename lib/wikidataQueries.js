...
function contextString (
  ...
  const companySuffix = position && company ? `at ${company}` : '';
  const dateRange = `${dateOne ? `from ${dateOne}` : ''} ${
    dateTwo ? `to ${dateTwo}` : ''
  }`;
...
module.exports = {
  ...
  extractClaimValue,
  async extraContext (qualifiersArr) {
    ...
    for (const qualifier of qualifiersArr) {
      ...
      if (qualifier.qualifiers?.P642) {
        ...
        context = [...context, { value: contextData.context }];
      } else {
