  ...
  extractNestedQCodeData: async (
    ...
    if (qCodeArr) {
      const promises = qCodeData.map(async (code) => {
        ...
        // if is not related in our systems + has param in config to hide if not related
      ...
      // specifically for non related items
      const hideFromUi = !related && hasDisplayLinkedFlag;
