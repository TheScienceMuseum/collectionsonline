      ...
      for (const qCodeKey of qCodeConfig) {
        const claimValue =
          entities[qCodeData]?.claims[qCodeKey]?.[0]?.mainsnak?.datavalue
            ?.value;
        // extracts only text properties from response
        if ((claimValue && claimValue[0]) || (claimValue && claimValue.text)) {
...
module.exports = {
  ...
  extractNestedQCodeData: async (qCodeData, elastic, config) => {
    const results = [];
    ...
    if (qCodeArr) {
      ...
      const flattenedResults = resolvedPromises.flat();
