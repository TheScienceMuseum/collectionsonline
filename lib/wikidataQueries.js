...
async function fetchImageMetadata (filename) {
  ...
  try {
    ...
    if (!meta) return null;
    const width = imageinfo.width || null;
    const height = imageinfo.height || null;
...
function generateString (data, qCode, key) {
  ...
  const imageUrl = imgPath
    ? `https://commons.wikimedia.org/w/index.php?title=Special:Redirect/file/${imgPath}&width=800`
    : null;
  return imageUrl;
}
        ...
        if (contextData.context) {
          context = [
            ...context,
            {
              ...
              ...(list && { list: true }),
              ...(contextValue && { label: contextValue }),
