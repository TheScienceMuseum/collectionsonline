...
// Returns an opening or closing paren character only when at least one date is present.
// formatType should be either '(' or ')'.
function conditionalParen (dateOne, dateTwo, formatType) {
...
function contextString (
  ...
  // If there is no label information at all, don't render an orphaned date range
  if (!company && !contextVal) return null;
  ...
  let dateRange;
  if (dateOne || dateTwo) {
    dateRange = `${conditionalParen(dateOne, dateTwo, '(')}${
      dateOne ? `${dateOne}` : ''
    }${dateOne && dateTwo ? '-' : ''}${
      dateTwo ? `${dateTwo}` : ''
    }${conditionalParen(dateOne, dateTwo, ')')}`;
...
// Batch-fetches labels for multiple Q-codes.
// Uses getManyEntities (always returns an array) to split automatically into ≤50-ID
// batches as required by the Wikidata API. Only requests 'labels' — we never need
// the full claims of nested entities, and requesting them bloats the response to
// several MB which reliably hits the 15 s timeout.
// Returns a merged map of { [qCode]: entityData } for label lookup without further calls.
async function batchFetchEntities (qCodes) {
  if (!qCodes || qCodes.length === 0) return {};
  const unique = [...new Set(qCodes.filter(Boolean))];
  if (unique.length === 0) return {};
  const urls = wbk.getManyEntities(unique, ['en'], ['labels'], 'json');
  const signal = AbortSignal.timeout(15000);
  const results = await Promise.allSettled(urls.map(url => wikibaseCall(url, signal)));
...
async function fetchImageMetadata (filename) {
  ...
  const url = 'https://commons.wikimedia.org/w/api.php?action=query&titles=' + encodedTitle +
  ...
  try {
    ...
    //   4. Normalise whitespace, then truncate to 100 characters with an ellipsis
    //      so long bibliographic descriptions don't overflow the image caption area.
    ...
    if (caption) {
      if (/&[lg]t;|&#\d+;/i.test(caption) || /https?:\/\//i.test(caption)) {
        ...
        caption = caption.slice(0, 100).trimEnd() + '\u2026';
      }
    }
    ...
    // filtering. Apply the same 100-char truncation for consistency.
    if (!caption && meta.ObjectName && meta.ObjectName.value) {
      const objectName = meta.ObjectName.value.replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim();
      if (objectName) {
        caption = objectName.length > 100 ? objectName.slice(0, 100).trimEnd() + '\u2026' : objectName;
      }
    }
    ...
    return {
      ...
      licenseUrl,
      artist,
      // true when the licence requires attribution (any CC BY variant)
      requiresAttribution: !!(license && license.toUpperCase().includes('BY')),
      commonsUrl: 'https://commons.wikimedia.org/wiki/File:' + filename.replace(/ /g, '_')
    };
  } catch (err) {
...
module.exports = {
  ...
  extractClaimValue,
  async extraContext (qualifiersArr, elastic, config, list = false, prefetchedEntities = null, relatedMap = null) {
  ...
  async extractNestedQCodeData (
    ...
    const processCode = async (code) => {
      ...
      // check if related record with ES search (or map lookup when pre-batched)
      const related = isQCodeRegex
        ? (relatedMap
            ? (relatedMap.get(code.value || code.id) || null)
