...
module.exports = (data, config, relatedItems, relatedAIItems, childRecordsList) => {
  ...
  // remove restricted (incl. 'complete' / 'incomplete') materials from record
  // Added Jan 2026, may want to re-visit / remove in future
  filterRestrictedMaterials(attributes);
...
function filterRestrictedMaterials (attributes) {
  if (attributes.material && Array.isArray(attributes.material)) {
    attributes.material = attributes.material.filter(material => {
      if (!material || typeof material !== 'object' || !material.value) return true;
      return !restrictedMaterials.some(restricted =>
        material.value.toLowerCase() === restricted.toLowerCase()
      );
    });
  }
}
    ...
    if ((data && data[key]) || (data && data.content && data.content[key])) {
      ...
      dk.forEach((el, i) => {
        ...
        if (el['@admin']) {
          ...
          if (!relationships[externalKey].data.some((r) => r.id === rel.id && r.type === rel.type)) {
  ...
  if (data && data.creation) {
    ...
    data.creation.maker?.forEach((el) => {
      ...
      if (el['@admin']) {
        ...
        if (!makers.data.some((m) => m.id === maker.id && m.type === maker.type)) {
    ...
    data.creation.place?.forEach((el) => {
      ...
      if (el['@admin']) {
        ...
        if (!place.data.some((pl) => pl.id === p.id && pl.type === p.type)) {
          place.data.push(p);
        }
      }
    });
...
function getChildRecords (data, childRecordsArr, config) {
  ...
  const childData = records
    .map((record) => {
      ...
      // is there a match between two data sets?
...
function filterAndExtractChildEnhancements (childRecords) {
  ...
  for (const child of childRecords) {
    ...
    enhancement.web.forEach((item) => {
      if (item && item.platform) items.push(item);
    });
  }
  return items.length ? { web: items } : null;
}

// pulls up wikidata from response
function getWikiData (data) {
  const { wikidata } = data._source;
  return wikidata ? { wikidata } : undefined;
}
