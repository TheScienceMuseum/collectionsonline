...
module.exports = (data, config, relatedItems) => {
  ...
  const relationships = getRelationships(data._source, relatedItems);
  const included = getIncluded(data._source, relationships, config, relatedItems);
...
function getRelationships (data, relatedItems) {
  const relationships = {};
...
function getIncluded (data, relationships, config, relatedItems) {
  if (relationships) {
    var included = [];

    Object.keys(relationships).forEach((key) => {
      if (key !== 'objects' && key !== 'documents') {
        ...
        resources.data.forEach((res) => {
          ...
          if (
            ...
            getNestedProperty(data, 'lifecycle.creation.0.places').some(el => el.admin.uid === res.id))
          ) {
    ...
    if (relatedItems) {
      var allRelated = relatedItems.relatedObjects.concat(relatedItems.relatedDocuments);
      included = included.concat(allRelated.map(el => {
        return {
          type: el.type,
          id: el.id,
          attributes: {
            summary_title: el.summary_title
          },
          links: {
            self: el.link
          }
