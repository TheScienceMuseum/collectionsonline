...
function getRelationships (data, relatedItems, type) {
  ...
  if (relatedItems && type === 'people') {
    ...
    relationships.children.data = relatedItems.map(returnRelationship);
  }
...
function getIncluded (data, relationships, config, relatedItems, type) {
  if (relationships) {
    ...
    Object.keys(relationships).forEach((key) => {
      if (key !== 'objects' && key !== 'documents' && key !== 'children') {
    ...
    if (relatedItems) {
      var allRelated = [];
      if (type === 'people') {
        allRelated = relatedItems.relatedObjects.concat(relatedItems.relatedDocuments);
      } else if (type === 'documents') {
        allRelated = relatedItems;
      }
      included = included.concat(allRelated.map(el => {
        return {
          ...
          id: el.id || TypeMapping.toExternal(el._id),
          attributes: {
            summary_title: el.summary_title || el._source.summary_title
          },
          links: {
            self: el.link || config.rootUrl + '/' + TypeMapping.toExternal(el._type) + '/' + TypeMapping.toExternal(el._id)
          }
        };
      }));
    }
