...
function getIncluded (data, relationships, config) {
  if (relationships) {
    ...
    Object.keys(relationships).forEach((key) => {
      let resources = relationships[key] || {data: []};
      ...
      resources.data.forEach((res) => {
        ...
        if (
          ...
          propertyExists(data, 'lifecycle', 'creation', '0', 'places', '0', 'admin', 'uid') &&
          data.lifecycle.creation[0].places[0].admin.uid === res.id)
        ) {
          ...
          if (TypeMapping.toExternal(el.admin.uid) === resource.id) {
            Object.keys(el).forEach((key) => {
              if (key !== '@link' && key !== 'admin') {
                ...
                resource.attributes.role = el[key].role[0].value;
              }
            });
          }
        });
