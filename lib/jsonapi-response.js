...
module.exports = (data, config, relatedItems) => {
  ...
  // see issue #176
  attributes.webDescription = getApprovedDescription(attributes);
...
function getApprovedDescription (attributes) {
  var description;
  if (attributes.options) {
    if (attributes.options.option1) {
      description = [{value: attributes.options.option1}];
    }
  }
    ...
    Object.keys(relationships).forEach((key) => {
      ...
      if (key !== 'objects' && key !== 'children' && key !== 'siblings' && key !== 'documents') {
        ...
        resources.data.forEach((res) => {
          ...
          resource.links.self = config.rootUrl + '/' + resource.type + '/' + resource.id + slugValue ? ('/' + slugValue) : '';
