...
module.exports = (data, config, relatedItems) => {
  ...
  const parentLink = TypeMapping.toExternal(
    ...
    // console.log("checking the parentLink", parentLink);

  // const childRecord = TypeMapping.toExternal(
  //   data._source.grouping && data._source.grouping[0]['@link']?.type
  // );

  // const viewTypeTwo = TypeMapping.toExternal(data._source.grouping);
  // const parentRecord = TypeMapping.toExternal(
  //   data._source['@datatype']?.grouping
  // );
  const record = getRecordType(data);
  // console.log(checking the viewType)
  ...
  return {
    data: {
      ...
      links,
      relationships,
      // ...(condition ? {childRecord} : {parentRecord}),
      // childRecord,
      // parentRecord
      record
    },
  ...
  multimedia = multimedia.map((e) => {
    if (e.processed) {
      ...
      Object.keys(processed).forEach((k) => {
        if (k === 'zoom') {
          // processed[k].location = config.iiifPath + '%2Fs3-zooms/' + encodeURIComponent(processed[k].location);
...
function getLinks (config, type, id, relationships, title, parentTitle) {
  ...
  if (relationships && relationships.parent) {
    ...
    links.parentSlug = `${config.rootUrl}/${relationships.parent.data[0].type}/${relationships.parent.data[0].id}/${slug(parentTitle).toLowerCase()}`;
  }
  // console.log("checking the relationships", relationships.parent)
  // console.log("checking the parent", `${links.parent}/${slug(parentTitle).toLowerCase()}`);
  return links;
}
