...
// TODO: This needs to be changed to work with mgroups
function getChildRecords(data, childRecordsArr, config) {
  ...
  const { base: parentType } = data['@datatype'];
  // console.log(data, 'cjeckinhg data');
  const childRecords =
  ...
  const childData = records
    .map((record) => {
      ...
      if (match) {
        ...
        // console.log(match, ':cechjking match');
        const id = match._source['@admin'].uid;
        const searchType = parentType === 'group' ? parentType : 'objects';
        const links = {
          self: `${config.rootUrl}/${searchType}/${id}/${slug(
            title[0].value
          ).toLowerCase()}`,
        };
        ...
        let multimedia = match._source.multimedia
          ? getImagePaths({ multimedia: match._source.multimedia }, config)
          : null;
    ...
    .filter(Boolean);
  // console.log(
  //   childData.map((child) => child.data.attributes),
  //   'checking child data'
  // );
  return childData;
}
