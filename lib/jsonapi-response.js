...
module.exports = (data, config, relatedItems) => {
  ...
  const links = getLinks(config, type, id, relationships, attributes.summary_title);
  ...
  const ___comment = '### WARNING ### - <enhancement> tags are experimental, please do not aggregate';

  return { data: { ___comment, type, id, attributes, links, relationships }, included, inProduction };
};
    ...
    if (e.processed) {
      ...
      Object.keys(processed).forEach(k => {
        if (k === 'zoom') {
          ...
          processed[k].location = config.iiifPath + encodeURIComponent(processed[k].location);
...
function getIncluded (data, relationships, config, relatedItems, type) {
  if (relationships) {
    ...
    Object.keys(relationships).forEach((key) => {
      ...
      if (key !== 'objects' && key !== 'children' && key !== 'siblings') {
        ...
        resources.data.forEach((res) => {
          ...
          resource.links = {};
          let slugValue = resource.attributes.summary_title && slug(resource.attributes.summary_title).toLowerCase();
    ...
    if (relatedItems) {
      const allRelated = Object.keys(relatedItems).map(el => {
...
function getCreationDetails (data) {
  if (data && data.creation) {
    ...
    data.creation.place?.forEach((el) => {
      const p = {};
      if (el['@admin']) {
        p.type = el['@admin'].id && TypeMapping.toExternal(el['@admin'].id.split('-')[0]);
