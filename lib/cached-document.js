...
async function getFullArchive (elastic, id) {
  try {
    ...
    if (result.body._source.level && result.body._source.level.value === 'fonds') {
      fond = {
        id,
        summary_title: result.body._source.summary_title,
      ...
      fond = {
        ...
        summary_title: result.body._source.fonds[0].summary_title
      };
    } else {
      return [{ id, summary_title: result.body._source.summary_title }];
    }
