...
module.exports = async (elastic, id, fondsId) => {
  try {
    ...
    // need to check why catbox isn't returning a proper error code? Then remove err.code === undefined
    if (err && err.code === 'ECONNREFUSED' || err.code === undefined) {
      const data = await getFullArchive(elastic, id);
...
async function cacheDocument (elastic, cache, id, data) {
  try {
    await cache.set({ segment: 'documents', id: id }, data, 100000000);
    await cache.stop();
