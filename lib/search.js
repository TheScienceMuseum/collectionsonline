...
const getCount = require('./scores.js').getCount;
/**
...
module.exports = (elastic, queryParams, next) => {
  ...
  const counts = [{count: 0, weight: 1}, {count: 10000, weight: 1.025}, {count: 100000, weight: 1.1}, {count: 1000000, weight: 1.2}];

  counts.forEach(el => getCount(el.count, function (err, data) {
    if (err) console.log(err);
    searchOpts.body.query.function_score.functions.push({filter: {term: {'admin.uid': data}}, weight: el.weight});
  }));
