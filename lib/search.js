...
const getScore = require('./get-score.js');
/**
...
module.exports = (elastic, queryParams, next) => {
  ...
  getScore(10000, 100000, function (err, data) {
    if (err) console.log(err);
    searchOpts.body.query.function_score.functions.push({filter: {term: {'admin.uid': data}}, weight: 1.025});
  });

  getScore(100000, 1000000, function (err, data) {
    if (err) console.log(err);
    searchOpts.body.query.function_score.functions.push({filter: {term: {'admin.uid': data}}, weight: 1.1});
  });

  getScore(1000000, function (err, data) {
    if (err) console.log(err);
    searchOpts.body.query.function_score.functions.push({filter: {term: {'admin.uid': data}}, weight: 1.2});
  });
