...
const BRAND = 'Science Museum Group Collection';
...
function truncateTitle (name) {
  const maxNameLength = MAX_TITLE_LENGTH - (' | ' + BRAND).length;
  if (name.length > maxNameLength) {
    return name.substring(0, maxNameLength - 1).trim() + '\u2026';
  }
  return name;
}
...
module.exports = function (q, selectedFilters, type) {
  ...
  if (!q && name) {
    return truncateTitle(name) + ' | ' + BRAND;
  }

  if (q) {
    return truncateTitle('Search for ' + q) + ' | ' + BRAND;
  }

  return `${defaultText(type)} | ${BRAND}`;
};
